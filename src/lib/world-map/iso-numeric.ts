 ...
 * Source: ISO 3166-1 (public). Used to map the world-atlas topojson's
 * `id` field (numeric) onto our country-data keys (alpha-2 uppercase).
 */

export const ISO_NUM_TO_A2: Readonly<Record<string, string>> = {
  ...
  // Disputed / world-atlas extras
  "-99": "XX",
};

/** Convert a numeric ID (string, possibly missing leading zeros) to alpha-2. */
export function numericToAlpha2(numeric: string | number): string | null {
  const padded = String(numeric).padStart(3, "0");
  return ISO_NUM_TO_A2[padded] ?? null;
}
